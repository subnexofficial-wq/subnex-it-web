...
import { Pencil, Trash2, Plus, Package, Tag, CheckCircle2, XCircle } from "lucide-react";
    ...
    <div className="p-4 sm:p-6 bg-gray-50 min-h-screen font-sans">
      <div className="container mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div>
            <h1 className="text-xl font-black text-gray-900 tracking-tight uppercase">Products</h1>
            <p className="text-xs text-gray-500 font-medium uppercase tracking-widest">Inventory Management</p>
          </div>
          <Link
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
            {products.map((p) => {
              ...
              return (
                <div key={p._id} className="group bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-lg transition-all duration-300 flex flex-col overflow-hidden relative">
                   
                  <div className="absolute top-2 right-2 z-10">
                    {p.active ? (
                      <CheckCircle2 size={14} className="text-green-500 bg-white rounded-full" />
                    ) : (
                      <XCircle size={14} className="text-red-400 bg-white rounded-full" />
                    )}
                  </div>
                  ...
                  <div className="p-3 flex-1 flex flex-col">
                    ...
                    <div className="mt-auto">
                      <div className="flex items-baseline gap-1">
                        <span className="text-sm font-black text-gray-900">৳ {minPrice}</span>
                        <span className="text-[12px] text-gray-400 font-bold uppercase">Start</span>
                      </div>
        ...
        {products.length === 0 && !loading && (
          <div className="py-20 text-center bg-white rounded-3xl border border-dashed">
             <p className="text-gray-400 font-bold uppercase text-[10px] tracking-widest">Empty Inventory</p>
          </div>
        )}
      </div>
